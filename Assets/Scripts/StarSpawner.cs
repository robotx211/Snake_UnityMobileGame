...
public class StarSpawner : MonoBehaviour {
	...
	//spawns stars at set intervals, or at random intervals within set parameters

	public int gridSizex = 19;
	public int gridSizey = 29;

	//(0, 0) is the centre, so the food can be spawned at (-9 -> +9, -14 -> +14)
	//(+ve, +ve) is top right
	...
	public void SpawnStar () {
		...
		//send send at ray at the location to check if a collider is there, or in the area
		...
		//quick extra check
		if (valid == true) {
			Instantiate(starPrefab, new Vector2 ((xPos * 0.56f), (yPos * 0.56f)), Quaternion.identity);
		}
	}
